...
#define PRESCALER	3
#define SCALED_CLOCK_SPEED	(F_CPU << PRESCALER)
#define CLOCK_TICKS	(SCALED_CLOCK_SPEED / 40)
//#define CLOCK_TICKS	(F_CPU / 40)
...
//  memset(_dmxData, 0, sizeof(_dmxData));
  ...
  if (_dmxMode == DMXController) {
	...
	timer1_attachInterrupt((timercallback)&transmit);
	timer1_enable(TIM_DIV265, TIM_EDGE, TIM_LOOP);
	timer1_write(CLOCK_TICKS);
...
void _DMXSerialBaud(uint16_t baud_setting, uint8_t format)
{
  ...
  Serial1.begin(baud_setting, format);
